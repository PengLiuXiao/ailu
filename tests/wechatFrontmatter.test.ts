...
describe('WeChat publishing metadata', () => {
  ...
  test('ignores incomplete metadata without an Ailu draft id', () => {
    expect(parseWeChatPublishState({
      [WECHAT_CONTENT_HASH_FRONTMATTER_KEY]: 'a'.repeat(64),
    })).toBeNull();
  });

  test('does not accept pre-Ailu draft linkage fields', () => {
    expect(parseWeChatPublishState({
      'retired-wechat-draft-id': 'draft_123',
      'retired-wechat-content-hash': 'a'.repeat(64),
      'retired-wechat-published-at': '2026-07-30T03:00:00.000Z',
      'retired-wechat-article-url': 'https://mp.weixin.qq.com/s/article-id',
    })).toBeNull();
  });
  ...
  test('pins the first renderer contract', () => {
    expect(WECHAT_RENDERER_VERSION).toBe('ailu-wechat-v2');
  });
});
