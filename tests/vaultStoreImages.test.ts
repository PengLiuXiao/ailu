...
describe('VaultStore generated images', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ailu-image-artifact-'));
  ...
  test('validates and copies an image into a conversation-scoped Vault path', async () => {
    ...
    const imported = await store.importGeneratedImage('conversation/unsafe', {
      itemId: 'image-1',
      sourcePath,
      mimeType: 'image/png',
      ...
      remainingTurnBytes: 64 * 1024 * 1024,
    });
    const { artifact } = imported;

    expect(imported.byteLength).toBe(8);
    ...
    expect(artifact.vaultPath)
      .toMatch(/^\.ailu\/generated-images\/conversation_unsafe\/.+\.png$/);
    ...
    expect(adapter.directories).toContain('.ailu');
    expect(adapter.texts.has('.ailu/conversations.json')).toBe(true);
    expect(adapter.directories).not.toContain('.retired-plugin');
  });
  ...
  test('rejects an exhausted or undersized turn budget before any Vault materialization', async () => {
    const adapter = new MemoryAdapter();
    const store = new VaultStore(adapter as unknown as DataAdapter);
    ...
    await expect(store.importGeneratedImage('conversation', {
      ...
      maxItemBytes: 25 * 1024 * 1024,
      remainingTurnBytes: 7,
    })).rejects.toThrow('remaining artifact budget');
    await expect(store.importGeneratedImage('conversation', {
      itemId: 'exhausted',
      sourcePath,
      mimeType: 'image/png',
      ...
      maxItemBytes: 25 * 1024 * 1024,
      remainingTurnBytes: 0,
    })).rejects.toThrow('budget is exhausted');

    expect(adapter.binaries.size).toBe(0);
