...
export class FeishuSharePanel {
  ...
  private async load(verifyCapabilities = false): Promise<void> {
    ...
    try {
      ...
      this.error = this.connection.status === 'error'
        ? userFacingErrorText(this.connection.message, '检测飞书连接失败，请手动重新检查。')
        : null;
    } catch (error) {
      this.error = userFacingErrorMessage(error, '加载飞书发布状态失败，请稍后重试。');
    } finally {
      if (!this.disposed) {
        this.loaded = true;
        this.loading = false;
  ...
  private renderLoading(parent: HTMLElement): void {
    const loading = parent.createDiv({ cls: 'ailu-share-loading' });
  ...
  private renderDisconnected(parent: HTMLElement): void {
    ...
    const statusRow = parent.createDiv({ cls: 'ailu-feishu-cli-status' });
    ...
    const primary = buttonWithIcon(
      parent,
      'ailu-share-primary-button is-feishu',
      missingCli
        ? '查看飞书 CLI 安装指引'
        : '连接飞书',
    ...
    parent.createDiv({
      cls: 'ailu-feishu-security-note',
      text: 'Ailu 不会安装或更新飞书 CLI；凭据由飞书 CLI 与系统安全存储管理',
    });
  }
  ...
  private renderAuthProgress(parent: HTMLElement): void {
    ...
    if (progress.phase === 'configuring') {
      ...
      if (progress.verificationUrl) {
        const open = buttonWithIcon(
          parent,
          'ailu-share-secondary-button ailu-feishu-full-button',
          '在浏览器中打开配置',
          'external-link',
        );
    ...
    if (progress.phase === 'failed' || progress.phase === 'cancelled') {
      if (progress.phase === 'cancelled') {
        this.renderError(parent, userFacingErrorText(progress.message, '飞书授权失败，请重新授权。'));
      } else {
    ...
    if (progress.qrCodeDataUrl && progress.verificationUrl) {
      ...
      const authCopy = authGrid.createDiv({ cls: 'ailu-feishu-auth-copy' });
    ...
    const separator = parent.createDiv({ cls: 'ailu-feishu-separator' });
    separator.createEl('strong', { text: '本次授权' });
    this.renderCapabilityList(parent);
    parent.createDiv({
      cls: 'ailu-feishu-auth-footnote',
  ...
  private renderConnected(parent: HTMLElement): void {
    ...
    const account = parent.createDiv({ cls: 'ailu-feishu-account-card' });
    const logoWrap = account.createDiv({
      cls: 'ailu-feishu-account-logo',
      attr: { 'aria-label': '飞书账号' },
    });
    setIcon(logoWrap, 'file-text');
    const copy = account.createDiv({ cls: 'ailu-feishu-account-copy' });
    ...
    const actions = account.createDiv({ cls: 'ailu-feishu-account-actions' });
    ...
    const granted = parent.createDiv({ cls: 'ailu-feishu-granted-row' });
    ...
    const form = parent.createDiv({ cls: 'ailu-feishu-publish-form' });
    ...
    const updateRow = form.createEl('label', { cls: 'ailu-feishu-checkbox-row' });
  ...
  private renderPublishedLink(parent: HTMLElement): void {
    ...
    const row = parent.createDiv({ cls: 'ailu-share-link-actions' });
    const open = buttonWithIcon(
      row,
      'ailu-share-secondary-button',
      '打开文档',
      'external-link',
    );
  ...
  private renderDuplicate(parent: HTMLElement): void {
    const error = parent.createDiv({ cls: 'ailu-share-error' });
    ...
    const publish = parent.createEl('button', {
      cls: 'ailu-share-primary-button is-feishu',
      text: '作为新的飞书文档发布',
      attr: { type: 'button' },
    });
  ...
  private renderAdminAction(parent: HTMLElement): void {
    this.renderFeishuHero(
      ...
      userFacingErrorText(
        ...
        '当前飞书应用缺少所需权限，请完成企业权限配置。',
      ),
    );
    const warning = parent.createDiv({ cls: 'ailu-share-warning-note' });
  ...
  private renderOperation(parent: HTMLElement): void {
    const operation = parent.createDiv({ cls: 'ailu-share-operation' });
    ...
    this.renderFeishuHero(
      ...
      expired
        ...
        : rejected
          ? '尚未同意全部权限'
          : '飞书授权暂未完成',
      userFacingErrorText(progress.message, '飞书授权失败，请重新授权。'),
      false,
    );
  ...
  private renderFeishuHero(
    ...
    const hero = parent.createDiv({ cls: 'ailu-feishu-hero' });
    if (showLogo) {
      const logo = hero.createDiv({
        cls: 'ailu-feishu-logo',
        attr: { 'aria-label': '飞书文档' },
      });
      setIcon(logo, 'file-text');
    }
    const copy = hero.createDiv({ cls: 'ailu-feishu-hero-copy' });
    copy.createEl('strong', { text: title });
    copy.createSpan({ text: description });
  }

  private renderCapabilityStrip(parent: HTMLElement): void {
    const strip = parent.createDiv({ cls: 'ailu-feishu-capability-strip' });
  ...
  private renderCapabilityList(parent: HTMLElement): void {
    const list = parent.createDiv({ cls: 'ailu-feishu-capability-list' });
  ...
  private renderAuthStepper(parent: HTMLElement, phase: FeishuAuthProgress['phase']): void {
    const stepper = parent.createDiv({ cls: 'ailu-feishu-stepper' });
    ...
    steps.forEach((step, index) => {
      const item = stepper.createDiv({
        cls: `ailu-feishu-step${step.done ? ' is-done' : ''}${step.active ? ' is-active' : ''}`,
      });
      ...
      if (index < steps.length - 1) stepper.createDiv({ cls: 'ailu-feishu-step-line' });
    });
  }

  private renderSpinnerStatus(parent: HTMLElement, text: string): void {
    const status = parent.createDiv({ cls: 'ailu-feishu-waiting' });
  ...
  private renderWarnings(parent: HTMLElement, snapshot: FeishuSnapshot): void {
    ...
    const warnings = parent.createDiv({ cls: 'ailu-share-warning-note' });
  ...
  private async finishAuthorization(): Promise<void> {
    try {
      if (!this.authAttemptId) throw new Error('飞书授权请求已失效，请重新发起。');
      this.connection = await this.options.cli.completeAuthorization(this.authAttemptId);
      this.authAttemptId = null;
      this.authProgress = null;
      this.error = null;
  ...
  private async disconnect(): Promise<void> {
    const confirmed = await confirmShareAction(this.options.app, {
      title: '断开飞书连接？',
      message: 'Ailu 将清除当前飞书用户授权，已发布的飞书文档会继续保留。',
      confirmText: '断开连接',
      dangerous: true,
    });
    if (!confirmed) return;
  ...
  private findDuplicatePath(documentId: string): string | null {
    for (const candidate of this.options.app.vault.getMarkdownFiles()) {
      ...
      if (state?.documentId === documentId) return candidate.path;
    }
    return null;
  }
