...
class GenerateWeChatCoverModal extends Modal {
  ...
  private render(): void {
    ...
    contentEl.empty();
    contentEl.addClass('ailu-cover-generator-modal');
    contentEl.createEl('h2', { text: 'AI 生成公众号封面' });

    if (this.checking) {
      contentEl.createDiv({
        cls: 'ailu-cover-generator-status',
    ...
    if (configError) {
      contentEl.createEl('p', {
        cls: 'ailu-cover-generator-error',
        text: configError,
      });
    ...
    contentEl.createEl('p', {
      cls: 'ailu-cover-generator-intro',
    ...
    actions.settingEl.addClass('ailu-cover-generator-actions');
    actions
      .addButton(button => {
    ...
    try {
      ...
      await this.options.runtimeManager.runTurn({
        ...
        if (event.type === 'artifact' && event.artifact.kind === 'image') {
          ...
          runtimeError = userFacingErrorText(
            [event.message, event.detail].filter(Boolean).join('：'),
            'Codex 生成封面失败，请稍后重试。',
          );
        }
      });
      ...
      this.error = userFacingErrorMessage(error, '封面生成失败，请稍后重试。');
      this.generating = false;
      if (this.contentEl.isConnected) this.render();
    } finally {
