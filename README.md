...
Ailu 0.3.1 以公开源码和 [GitHub Release](https://github.com/PengLiuXiao/ailu/releases/tag/0.3.1) 分发。普通用户优先安装 Release 中经过验证的 `main.js`、`manifest.json` 和 `styles.css`；GitHub 自动生成的 “Source code” 压缩包不包含 `main.js` 与 `build-attestation.json`，不能直接当作 Obsidian 插件包使用。
...
2. 从 [Ailu 0.3.1 Release](https://github.com/PengLiuXiao/ailu/releases/tag/0.3.1) 下载 `main.js`、`manifest.json` 和 `styles.css`。可同时下载 `build-attestation.json`、`LICENSE` 与 `THIRD_PARTY_NOTICES.md` 核对构建和许可证。
